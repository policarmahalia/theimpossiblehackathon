...
func _ready():
	...
	# starting states
	eyenstyne.position.y = -screen.y            # eye starts fully above screen
	...
	await eyenstyne_descend()   # eye comes down
	await flash_and_swap()      # flash + bunny transforms
	await eyenstyne_exit()      # eye continues off screen
...
func eyenstyne_descend():
	...
	tween.tween_property(eyenstyne, "position:y", 0, 2.0).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
...
func flash_and_swap():
	var tween = create_tween()
	tween.tween_property(flash, "modulate:a", 1.0, 0.1)                          # flash in
	tween.tween_callback(func(): bunny.texture = bunny_stand)                    # swap sprite while white
	tween.tween_property(flash, "modulate:a", 0.0, 0.2)                          # flash out
	await tween.finished


# eye continues moving down off screen
func eyenstyne_exit():
	var tween = create_tween()
	tween.tween_property(eyenstyne, "position:y", 400, 1.5).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
	await tween.finished
